...
// echoes. The showcase for `tape()`: a soft pad feeds a LONG echo loop, and the
// whole circulating wash runs through TAPE — so every pass comes back a little
// warmer, a little more warped (wow/flutter pitch-drift + saturation). The tape
// degradation IS the instrument; it's what turns a delay into an aging loop.
// Fifth box in the effects family — spacecho (echo) · cathedral (reverb) ·
...
//   tap the tape window or 1..6 to drop a note in by hand
...
void init(void) {
    ...
    instrument(PAD, INSTR_SAW, 220, 0, 5, 1500);   // soft swelling pad
    instrument_filter(PAD, FILTER_LOW, 1300, 2);   // warm
    instrument_harmonics(PAD, 0.4f);
