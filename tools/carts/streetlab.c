...
void update(void){
    ...
    if ((keyp('g')||keyp('G')) && !osmMode) field_roads = !field_roads;  // toggle field-based rendering (A/B); LOCKED on in OSM mode — the per-arm path assumes collinear arm-pairs, so it breaks on independent OSM arms
