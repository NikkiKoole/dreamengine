...
const MTX_LW = 40, MTX_HH = 12, COLS = 6, SCREEN = 320;
...
const FEATURES = ['bevel', 'outline', 'shadow', 'dither', 'boil'];   // cols 1..5 (col 0 = baseline)

// EXPECTED support per brush: which of [bevel,outline,shadow,dither,boil] SHOULD change the render.
// 1 = must apply, 0 = intentionally inert (N/A for this brush). This is the source of truth for INTENT.
const EXPECT = {
  ink:      [1,1,1,1,1], pencil: [1,1,1,1,1], liner: [1,1,1,1,1], marker: [1,1,1,1,1],
  chalk:    [1,1,1,1,1],
  sketch:   [0,0,0,0,1], spray:  [0,0,0,0,1], bristle:[0,0,0,0,1],
  paint:    [1,1,1,1,1],
  nib:      [1,1,1,1,1], brushpen:[1,1,1,1,1], reed:[1,1,1,1,1], twist:[1,1,1,1,1],
  oil:      [0,0,1,0,1],   // impasto: bevel is intrinsic (not a toggle), no outline/dither; drop-shadow + boil apply
};
