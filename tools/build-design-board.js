...
const { collectCartTodos } = require("./cart-todos.js");
...
const cartTodos = collectCartTodos().sort((a, b) => b.todos.length - a.todos.length || a.name.localeCompare(b.name));
const cartTodoTotal = cartTodos.reduce((n, c) => n + c.todos.length, 0);
...
// every phase is a <details> accordion; `open` = expanded by default. The
// actionable phases (ready / building / unclassified) open; the long tails
// (exploring / shipped / cut) start collapsed so the page opens on what's next.
const PH = {
...
function phaseBlock(k) {
  ...
  return `<section class="ph" style="--c:${ph.c}">
    <details${ph.open ? " open" : ""}><summary>${hd}</summary>${body}</details>
  </section>`;
}
...
function cartBlock(c) {
  return `<div class="ctc"><a class="cart" data-cart="${esc(c.name)}" href="#">${esc(c.name)} <span class="cn">${c.todos.length}</span></a>${c.todos.map(t => `<div class="ctodo">${esc(t)}</div>`).join("")}</div>`;
}
...
<div id="root">
  <header>
    <h1>TODO&nbsp;LIST</h1>
    <div class="count">${esc(summary)}<span class="sep">·</span>${board.length} designs<span class="sep">·</span>${adrs.length} ADRs<span class="sep">·</span>${cartTodoTotal} cart todos<span class="sep">·</span>${unmarked} unmarked${newest ? `<span class="sep">·</span>newest ${esc(fmtDate(newest))}` : ""}</div>
  ...
  <section class="ph" style="--c:var(--dim)">
    <details><summary><span class="lead">▸</span>DECISIONS <span class="n">${adrs.length}</span></summary><div class="chips">${adrChips}</div></details>
  </section>
  <section class="ph" style="--c:var(--yellow)">
    <details><summary><span class="lead">▸</span>CARTS <span class="n">${cartTodoTotal} todos · ${cartTodos.length} carts</span></summary>
    <p class="gloss">per-cart de:meta.todo[] polish backlog (the cart-todos.js stream). Click a cart to load it in the editor.</p>
    ${cartTodos.map(cartBlock).join("")}</details>
  </section>
...
/* every phase is a <details> accordion; the summary is the phase header */
...
.gloss{color:var(--dim);font-size:12px;margin:0 0 12px 18px;font-style:italic}
/* CARTS section — per-cart de:meta.todo[] backlog */
.ctc{margin:0 0 12px;border-left:2px solid transparent;padding-left:8px}
.ctc:hover{border-left-color:var(--c)}
a.cart{display:inline-block;text-decoration:none;color:var(--bright);font-weight:700;font-size:13px;margin-bottom:2px}
a.cart:hover{color:var(--c)}
a.cart .cn{color:var(--dim);font-weight:400;font-size:11px}
.ctodo{color:var(--dim);font-size:12px;line-height:1.45;margin-left:14px;padding:1px 0}
.ctodo::before{content:"• ";color:var(--c)}
