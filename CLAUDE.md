...
│   ├── spec.js         #   run each cart's spec() — the GAMEPLAY-LOGIC test gate (twin of
│   │                   #   tune-check, for game logic instead of audio). Compiles carts that
│   │                   #   define a spec() with -DDE_SPEC, runs it HEADLESS (step()/expect()
│   │                   #   drive update() + assert on state / pure fns), parses JSONL pass/fail.
│   │                   #   Carts WITHOUT a spec() are skipped (weak stub). `node tools/spec.js`
│   │                   #   = all; `<cart>` = one; --quiet exits 1 on failure (CI). Opt-in, manual.
│   │                   #   API in runtime/spec.h; streetlab is the reference. cart-analyze.js
│   │                   #   ranks which sims warrant one. Design: docs/design/spec-harness.md
...
- **After touching engine LEVELS or EFFECTS in `sound.h`** (gains, envelopes, amp-normalize, a shaper, any effect DSP): run the stability gates — `node tools/level-check.js --quiet` (loudness regression: an engine that got louder/quieter or now slams the limiter — exit 1 on >4 dB drift vs `tools/level-baseline.json`), `node tools/fx-check.js --quiet` (effect stability at extremes incl. STACKS via fx_order: NaN-collapse / DC runaway / self-oscillation — exit 1 on a regression vs `tools/fx-baseline.json`), and for a feedback-effect or voice-lifetime edit `node tools/soak-check.js --quiet` (~64s soak: voice leak / level drift / energy accumulation). For engine-math an optimizer/fast-math/libm change could compile differently on the web, `node tools/web-audio-check.js --quiet` (wasm-vs-native codegen parity: 15/16 engines sample-identical, BOWED chaotic-but-level-matched). All twins of tune-check, deterministic, re-bless an INTENDED change with `--save`. See [`docs/guides/debug-harness.md`](docs/guides/debug-harness.md) → "Level" / "Effect stability" / "Soak" and the findings in [`docs/design/audio-notes.md`](docs/design/audio-notes.md) §20 (BOWED/BRASS run hot — open; the phaser/echo/flanger DC at max feedback — FIXED; denormal FTZ — shipped).
...
- **Carts with GAME LOGIC can carry a `spec()`** — the gameplay twin of the audio gates: a per-cart, deterministic, HEADLESS assertion pass (`step(n)` drives `init()`+`update()`, `expect(cond,msg)` asserts on the resulting state or a pure helper), live only under `-DDE_SPEC` (zero footprint in a normal build — API in [`runtime/spec.h`](runtime/spec.h)). Run `node tools/spec.js` (every cart with one) / `node tools/spec.js <cart>` / `--quiet` (CI, exit 1 on any failure); carts without a `spec()` are skipped. Opt-in, manual — write one on the complex sims where a logic regression hurts (`node tools/cart-analyze.js` ranks candidates). `streetlab` is the reference (pins its pure geometry + a clamp). NOT for pixels (use `ui-audit`) or audio (the audio gates). Design + the `step()` vs `frame()` split: [`docs/design/spec-harness.md`](docs/design/spec-harness.md).
