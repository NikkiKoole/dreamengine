...
- **Effects are SET-AND-HOLD — (re)configure them only when a value CHANGES, never every frame.** Wiring a knob straight into `update()`/`draw()` so `crush()`/`tape()`/`eq()`/`chorus()`/`reverb()`… fires 60×/s reconfigures the bus DSP (ring buffers, coefficients) every frame, churning the audio thread → **stutter/lag, not a crash** (silent, easy to misattribute). Re-apply only when the value moved (keep a last-applied copy and compare, or gate on the `ui_*` "changed" return); same for live `note_cutoff`/`note_reverb`/`note_vol` (push only while the target is actually moving). Copy `groovebox`'s `apply_fx()`. Full note: [`docs/guides/effects-recipes.md`](docs/guides/effects-recipes.md) (intro callout).
...
- **Navigating C code: use the LSP tool** (clangd works on this repo) — `documentSymbol` on `studio.h` lists the whole API with signatures in one call; goToDefinition/findReferences/call-hierarchy resolve semantically (it won't confuse a local `grid` with the `map()` builtin like grep does). Caveat: it can't follow references *into* `sound.h` (only compiled inside `studio.c`).
  - **clangd must be ≥ 14 — fresh machine setup.** Carts resolve their includes via `tools/carts/compile_flags.txt` (relative `-I../../runtime`, portable — don't make it absolute). macOS's stock `/usr/bin/clangd` is **13**, which has a CWD bug that mis-resolves the relative `-I` → a wall of false "`studio.h` file not found" / "undeclared `CLR_*`/`SCREEN_W`" diagnostics on every cart. **The repo file is correct; the binary is the problem.** Fix per machine (keg-only brew llvm isn't on PATH by default): `brew install llvm && ln -sf "$(brew --prefix llvm)/bin/clangd" "$(brew --prefix)/bin/clangd"` — `/usr/local/bin` (or `/opt/homebrew/bin`) beats `/usr/bin` on PATH, so `clangd` then resolves to the modern build. Verify: `clangd --version` (want 19.x) and `clangd --check=tools/carts/<any>.c` (want "0 errors"). If you still see those false positives, you're on clangd 13 — fix the binary, don't touch the repo. For bulk "how often is X used across all carts" questions, run `node tools/api-usage.js` instead — it also cross-checks studio.h ↔ studioDocs.js ↔ shell.js coverage. Findings snapshot: [`docs/design/api-usage-audit.md`](docs/design/api-usage-audit.md).
