...
| `sensible-soccer.md` | Sensible Soccer (top-down football) | 🟡 | after-touch ball curve + team recolor |
| `operation-wolf.md` | Operation Wolf (light-gun shooter) | 🟢 | mouse light-gun + shoot-to-reload |
| `final-fight.md` | Final Fight (belt brawler) | 🟢 | reusable brawler engine; 2 stages + boss |
| `river-city.md` | River City (brawler RPG) | 🟡 | builds on final-fight + shops/stats/zones |
| `cannon-fodder.md` | Cannon Fodder (squad run-gun) | 🟢 | mouse squad + permadeath + explosions |
| `outrun.md` | OutRun (pseudo-3D racer) | 🟡 | branching road forks + biomes; extends `racer` |
| `pod-racer.md` | Pod Racer (textured-3D racer) | 🔴 | tritex canyon + boost/heat; one track |
| `excitebike.md` | Excitebike (+ level editor) | 🟢 | landing-angle physics + mouse track builder |
| `advance-wars.md` | Advance Wars (turn tactics) | 🟡 | damage table + terrain + capture; vs `fourx` |
...
| `pizza-tycoon.md` | Pizza Tycoon (business sim) | 🔴 | buy shops + recipe/price + mafia cards |
| `trade-winds.md` | Trade Winds (sail trading) | 🟡 | reuses Druglord economy + storms/pirates |
| `gta-turf.md` | GTA turf war | 🟡 | territory-capture sandbox; extends `gta` |
| `doom.md` | Doom (raycaster FPS) | 🟡 | billboard demons + weapons; extends `raycaster` |
| `blade-runner.md` | Blade Runner (noir adventure) | 🟡 | scenes/hotspots/dialogue; atmosphere-first |
| `mass-effect.md` | Mass Effect (sci-fi action) | 🔴 | hub + planet select + 1 mission + dialogue wheel |
| `heroes-of-might-magic.md` | Heroes of Might & Magic | 🔴 | adventure map + STACK battles + town; reuses `advancewars` |
| `hotline-miami.md` | Hotline Miami (top-down action) | 🟡 | one-hit-kill + instant restart; synthwave bed |
| `dungeon-keeper.md` | Dungeon Keeper (build/defend sim) | 🟡 | dig/claim + slap-hand + invasions; reuses `dune` |
| `mario-paint-sound.md` | Mario Paint Sound (TOOL: composer) | 🟢 | melodic stamp grid + looping playhead |
| `deluxe-paint.md` | Deluxe Paint (TOOL: paint program) | 🟡 | fillp patterns + dithered gradients + color cycling |
| `sky-strike.md` | Sky Strike (top-down airplane shmup) | 🟢 | weapon-upgrade tiers + scrolling tile world; vs `galaga`/`xevious` |
| `pinball.md` | Pinball (one table) | 🟢 | segment-reflection ball physics + flippers/multiball |
| `smooch-lounge.md` | Smooch Lounge (saucy cabaret rhythm) | 🟢 | lane-rhythm off beat() + flirt-meter; PG-13 innuendo |
| `skeleton-animator.md` | Bones (TOOL: stick-figure FK animator) | 🟢 | 18-bone FK skeleton + sequencer grid; hex cells + drag-pose |
