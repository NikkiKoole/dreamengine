...
|---|---|---|---|
| **An auto-composing station** (plays itself, you tune it) | `radio.h` (+ `solo.h` for a player jam strip, `improv.h` for an auto-soloist) | `bossa` (simplest), `cocktail`/`motorik` (improv.h soloist), `dub`/`citypop` (solo.h jam strip) | See [`game-music.md`](game-music.md) for the chord/clock brain. NEVER hand-roll the chassis. |
...
| **A classic-machine homage** (faithful hardware recreation) | `ui.h` faders/knobs + a step sequencer or keybed | drums: `tr808`/`tr909`/`cr78`; mono synth: `tb303`; keyboard synth: `sh101`; effect: `spacecho` | These are dense; pick the one whose *form factor* matches (drum grid vs faders vs keybed). |
