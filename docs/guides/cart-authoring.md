...
| `cursor.h` | a **pixel mouse cursor** drawn into the canvas: `cursor_draw(shape)` (`CUR_ARROW`/`HAND`/`GRAB`/`CROSS`/`MOVE`) + `cursor_draw_tint(shape, col)`. Auto-hides the OS pointer once a real mouse is seen and leaves it alone on touch. Call **last** in `draw()` | a **mouse-driven** cart (editor/tool/sandbox) that wants a pointer matching the pixel art — and one that **shows up in screenshots/clips** (the OS cursor never does, since the capture is the render texture). Tint a crosshair to the active tool. NOT needed on touch-only carts. Built on the engine `mouse_hide()`/`mouse_show()` primitives | `logic` |
