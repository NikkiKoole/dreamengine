...
│   ├── traffic-ai.md  PLAN: NPC driving behaviours as a TRAFFIC mode in trackgen (same track, not a race) — the decision (a mode, not a new cart, + the split trigger), and the 6 systems to test (lane-keep + car-following/IDM = the foundation, phantom jams, overtaking, figure-8 right-of-way, speed zones, hazard/merge), each with its ports-to-sloop + spec angle. Feeds sloop rung-4 NPC traffic + streetlab's signal-state/ped-logic gaps
