...
│   ├── spec-harness.md  DESIGN (not built): per-cart `spec()` — the gameplay twin of tune-check, a deterministic headless logic safety net (turns ad-hoc throwaway verification into a committed re-runnable artifact). Locked decisions (imperative, logic-only, opt-in/manual), two verbs (step() for stateful carts / frame()+probe for procedural) + the zero-authoring golden check; which carts warrant one via `node tools/cart-analyze.js` (complexity × global-state ranking — 0/394 use de_state, 174 stateful, 38 procedural)
