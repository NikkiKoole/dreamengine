...
│   ├── software-canvas.md  PROPOSAL (the fleet survey's #1 lever): rasterize the whole 64k-px frame on the CPU + UpdateTexture once/frame to kill rlVertex3f engine-wide (pset is 59.6k/frame); the design forks (RGBA vs index buffer, camera_ex zoom/rotation, compositing) + a staged DE_SOFTWARE_CANVAS prototype plan
...
│   ├── cityplan.md  ★ the lotfill + interiors MERGE — built in cart cityplan.c: a whole world (terrain/forest/farm/city) you LIFT THE ROOF off by zooming (city of roofs far → BSP floor-plans near → furniture closer; F = dollhouse). The scale decision, the compose order, and the staged build plan (A terrain+scatter+rows ✅ / B ruins 🔄 / C border-pave-stamp ⬜). The cart we develop forward; lotfill/interiors are now reference workbenches
