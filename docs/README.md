...
│   ├── field-based-road-rendering.md  QUEUED streetlab refactor (proven in the skewlab cart, NOT yet in streetlab): draw the road from ONE lateral distance field (latDist = ⊥ dist to nearest centreline) — asphalt/kerb/sidewalk/lane-lines all thresholds on it, clean + uniform at any SKEW or CURVE/Perlin, fixing the per-arm strays/missing-outline/double-lines. Clear boundary: render RULES port verbatim, field SOURCE is per-cart, dashes/arrows/junction-clearing stay a separate longitudinal layer. Would DELETE mirror_blit + casing-fillet + stroke_corner. Pairs with software-canvas.md (per-pixel)
