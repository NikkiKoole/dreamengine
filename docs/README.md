...
│   ├── external-data-carts.md  EXPERIMENT: a cart loads a data blob at RUNTIME (--data <file> → de_data_path() + runtime/json.h) instead of baking it into source — kills the floorwalker/seinelaan bake-a-cart-per-dataset smell. The shared "vector features" JSON schema, the OSM/Overpass fetcher (tools/osm-roads.js, with --demo), the roadview consumer cart, and where to get real road data. Not committed API
